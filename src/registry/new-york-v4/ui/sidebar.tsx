    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn(
            "group/sidebar-wrapper has-data-[variant=inset]:bg-sidebar flex min-h-svh w-full bg-card",
            className
          )}
          {...props}
        >
    ...
    >
      ...
      >
        <div
          ...
          className="bg-element-bg flex h-full w-full flex-col rounded-2xl shadow-neumorphic" // Apply Neumorphic styles
        >
          {children}
        </div>
      </div>
...
function SidebarGroupAction({
  ...
  return (
    <Comp
      ...
      className={cn(
        "text-text-primary ring-sidebar-ring hover:text-accent-primary active:shadow-neumorphic-inset absolute top-3.5 right-3 flex aspect-square w-5 items-center justify-center rounded-md p-0 outline-hidden transition-transform focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0", // Apply Neumorphic styles
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn(
        "border-border mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-l px-2.5 py-0.5", // Use border-border
        "group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        "text-text-primary ring-sidebar-ring hover:text-accent-primary active:shadow-neumorphic-inset flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 outline-hidden focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0", // Apply Neumorphic styles
        "data-[active=true]:bg-accent-primary data-[active=true]:text-white", // Apply Neumorphic active styles
        size === "sm" && "text-xs",
        size === "md" && "text-sm",
        "group-data-[collapsible=icon]:hidden",
        className
